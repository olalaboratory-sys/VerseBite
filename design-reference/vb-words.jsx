// vb-words.jsx — bilingual mini-dictionary + tokenizer for learning mode.
// Exports: VB_WORDS, vbTokenize, vbLookup
// VB_WORDS.en[word] = { def, ko, pos }   (English headword → meaning + Korean)
// VB_WORDS.ko[word] = { def, en, roman } (Korean headword → meaning + English)
...
const VB_WORDS = {
  en: {
    ...
    household:{def:'a family living together',ko:'집안 · 가정',pos:'noun'},
    ...
    rejoice:{def:'to feel and show great joy',ko:'즐거워하다',pos:'verb'},
...
// common Korean particles (josa) and endings to strip for matching
const KO_PARTICLES = ['에게는','에서','으로','로서','이라','라는','에게','께서','처럼','보다','마다','부터','까지','이나','으로','은','는','이','가','을','를','에','의','도','와','과','로','만','며','고','은','님','들','이여','여','라','한','할','하','함','이라도'];

// Tokenize text into [{text, word}] preserving spaces & punctuation.
function vbTokenize(text, lang) {
...
// Korean definitions for English headwords (shown in Learn English mode).
const VB_KODEF = {
...
// Look up a tapped token. Returns {headword, def, trans, roman, lang} or null.
function vbLookup(token, lang) {
  if (lang === 'en') {
    const k = token.toLowerCase().replace(/[^a-z']/g, '');
    const e = VB_WORDS.en[k];
    // Learn English → definition shown in Korean (VB_KODEF); English prose kept as secondary reference.
    if (e) return { headword: k, def: VB_KODEF[k] || e.ko, defAlt: e.def, trans: e.ko, roman: null, pos: e.pos, lang: 'en' };
    return null;
  }
  // Korean: exact, then strip particles, then prefix match
  const d = VB_WORDS.ko;
  ...
  // prefix: longest dictionary key that the eojeol starts with
  let best = null;
  for (const k in d) { if (k.length >= 2 && token.startsWith(k) && (!best || k.length > best.length)) best = k; }
  if (best) return mkKo(best, d[best]);
  // suffix stem fallback (token contains a key)
  for (const k in d) { if (k.length >= 2 && token.includes(k)) return mkKo(k, d[k]); }
  return null;
}
function mkKo(h, e){ return { headword: h, def: e.def, trans: e.en, roman: e.roman, pos: null, lang: 'ko' }; }

Object.assign(window, { VB_WORDS, vbTokenize, vbLookup });
