...
export async function fetchStudyGuide(verse: Verse): Promise<GeneratedGuide | null> {
  if (!CONFIG.studyEndpoint) return null;
  try {
    const res = await fetch(CONFIG.studyEndpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: verse.id, cat: verse.cat, refEn: verse.refEn, refKo: verse.refKo, en: verse.en, ko: verse.ko }),
    });
